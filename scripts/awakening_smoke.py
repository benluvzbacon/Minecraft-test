...
class AwakeningScenario:
    ...
    def handle(self,source,line):
        ...
        if 'RIFTBORN_20_DASH_OK' in line:
            self.cmd('riftborn_awake dash','tp RiftbornTester 0.5 325 0.5 -90 0','execute in riftborn:the_abyss run summon riftborn:abyssal_brute 4.5 325 0.5 {NoAI:1b,Silent:1b,PersistenceRequired:1b,Tags:["awakening_weapon"]}')
        ...
        if 'RIFTBORN_20_BOSS_COMBAT' in line:self.cmd('tp RiftbornTester 0.5 334 5.5 0 25','riftborn_awake combat')
        if 'RIFTBORN_20_BOSS_DEFEAT_REQUEST' in line:
            self.cmd('riftborn_awake defeat')
            if self.boss==0:
                self.cmd('execute in riftborn:the_abyss run setblock 0 325 8 riftborn:architect_altar','tp RiftbornTester 0.5 325 5.5 0 0');self.hand('riftborn:titan_core')
            elif self.boss==1:
                self.cmd('tp RiftbornTester 0.5 325 5.5 0 -8','riftborn_awake storm')
            self.boss+=1
        if 'RIFTBORN_20_STORM_OK' in line:
            self.cmd('riftborn_awake storm_check','execute in riftborn:the_abyss run setblock 0 325 8 riftborn:sovereign_altar','tp RiftbornTester 0.5 325 5.5 0 0');self.hand('riftborn:collapse_catalyst')
        if 'RIFTBORN_20_HERALD_DEFEAT_REQUEST' in line:self.cmd('riftborn_awake herald','tp RiftbornTester 0.5 325 5.5 0 0');self.hand('riftborn:sovereign_sigil')
        if 'RIFTBORN_20_FINAL_BOSS_OK' in line:self.cmd('riftborn_awake finish','tp RiftbornTester 0.5 325 -2.5 180 0');self.hand('minecraft:air')
    def verify(self):
