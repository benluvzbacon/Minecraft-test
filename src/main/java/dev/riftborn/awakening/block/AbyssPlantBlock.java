...
public final class AbyssPlantBlock extends PlantBlock {
    ...
    public AbyssPlantBlock(Settings s) {
        super(s);
    }
    @Override
    public MapCodec<AbyssPlantBlock> getCodec() {
        return CODEC;
    }
    @Override
    protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
        return floor.isOf(AbyssBlocks.STONE) || floor.isOf(AbyssBlocks.LUMINOUS_SHALE)
            || floor.isOf(ModBlocks.RIFT_STONE) || floor.isOf(Blocks.BLACKSTONE);
    }
    @Override
    protected void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        if (!world.isClient && this == AbyssBlocks.BRAMBLE && entity.age % 20 == 0)
